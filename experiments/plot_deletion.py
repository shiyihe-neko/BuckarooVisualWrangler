#!/usr/bin/env python3
"""
make_plots.py — read runtimes_removal.json and write PNG+SVG plots to ./plot

• 01_box_violin.{png,svg}   – side-by-side violin/box plots
• 02_median_error.{png,svg} – median lines with IQR error bars
• 03_speed_ratio.{png,svg}  – Postgres ÷ Pandas speed-up curve
"""
...
DATA_FILE = Path(f"results/{table_name}_runtimes_removal.json")
PLOT_DIR  = Path(f"results/plot_removal_{table_name}")


# ────────── helpers ────────────────────────────────────────────────────────────
def _save(fig: plt.Figure, stem: str) -> None:
    """Save *fig* as both PNG and SVG with the given filename stem."""
    for ext in ("png", "svg"):
...
def plot_box_violin(df: pd.DataFrame) -> None:
    ...
    ax.set(xscale="log", yscale="log",
           xlabel="Dataset size (#rows)", ylabel="Query time (s)",
           title="Row Removal Time")
    ax.legend(title="DBMS")
    _save(fig, "01_box_violin")


def plot_median_error(df: pd.DataFrame) -> None:
    """Median line with IQR bars."""
    summary = (
    ...
    ax.set(xscale="log",
           xlabel="Dataset size (#rows)", ylabel="Median query time (s)",
           title="Row Removal Time")
    ax.legend()
...
def plot_speed_ratio(df: pd.DataFrame) -> None:
    """Relative speed: Postgres ÷ Pandas (values <1 ⇒ Postgres faster)."""
